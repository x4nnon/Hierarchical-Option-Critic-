...
class Args:
    ...
    ent_coef_action: float = 0.025
    ent_coef_option: float = 0.3
    ent_coef_meta: float = 0.3
    clip_coef: float = 0.1
...
def conduct_evals(agent, writer, global_step_truth, run_name, device):
    ...
    current_meta_options = torch.full((args.eval_batch_size,), -1, dtype=torch.long, device=device)
        ...
        with torch.no_grad():
            ...
            for i in range(0, args.eval_specific_envs, args.eval_batch_size):
                ...
                for ts in range(args.max_eval_ep_len + 1):
                    ...
                    needs_new_meta_option = current_meta_options == -1  # Determine which environments need a new meta-option

                    new_meta_options, new_meta_option_logits, _ = agent.select_meta_option(state_rep)
                    current_meta_options[needs_new_meta_option] = new_meta_options[needs_new_meta_option]  # Update current meta-options
                    
                    needs_new_option = current_options == -1  # Determine which environments need a new option
                    
                    new_options, new_option_logits, _ = agent.select_option(state_rep, current_meta_options)
                    ...
                    with torch.no_grad():
                        ...
                        meta_terminated = torch.bernoulli(meta_termination_probs).bool().flatten()  # Sample meta-termination decisions
                        current_meta_options = torch.where(meta_terminated, torch.tensor(-1, device=device, dtype=torch.long), current_meta_options)
...
def hoc(args):
    ...
    meta_options_buffer = torch.zeros((args.num_steps, args.num_envs), dtype=torch.long).to(device)  # Track active meta-options
    ...
    logprobs_meta_options = torch.zeros((args.num_steps, args.num_envs)).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                logprobs_meta_options[step] = new_meta_option_logits
            ...
            meta_options_buffer[step] = current_meta_options  # Store the meta-option that was active during this step
        ...
        with torch.no_grad():
            ...
            meta_option_advantages = returns - values
        ...
        b_meta_options = meta_options_buffer.reshape((-1,) + envs.single_action_space.shape)
        b_advantages = advantages.reshape(-1)
        b_option_advantages = option_advantages.reshape(-1)
        b_meta_option_advantages = meta_option_advantages.reshape(-1)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, optionlogprobs, optionentropy = agent.select_option(update_state_rep, b_meta_options[mb_inds], b_options[mb_inds])
                _, meta_optionlogprobs, meta_optionentropy = agent.select_meta_option(update_state_rep, b_meta_options[mb_inds])

                newvalue = agent.compute_value(update_state_rep)

                mb_advantages = b_advantages[mb_inds]
                mb_o_advantages = b_option_advantages[mb_inds]
                mb_m_advantages = b_meta_option_advantages[mb_inds]

                # action Policy loss
                a_pg_loss = (-actionlogprobs * mb_advantages).mean()
                
                # option Policy loss
                o_pg_loss = (-optionlogprobs * mb_o_advantages).mean()

                # meta-option Policy loss
                m_pg_loss = (-meta_optionlogprobs * mb_m_advantages).mean()
                ...
                # meta-option termination loss
                meta_termination_probs = agent.meta_termination_function(update_state_rep, b_meta_options[mb_inds])
                meta_termination_advantage = b_returns[mb_inds] - newvalue.detach()
                meta_termination_loss = (meta_termination_probs * meta_termination_advantage).mean()

                a_entropy_loss = actionentropy.mean()
                o_entropy_loss = optionentropy.mean()
                m_entropy_loss = meta_optionentropy.mean()

                # # Clip losses
                max_loss_value = 0.5  # Example threshold, adjust as needed
                a_pg_loss = torch.clamp(a_pg_loss, -max_loss_value, max_loss_value)
                o_pg_loss = torch.clamp(o_pg_loss, -max_loss_value, max_loss_value)
                m_pg_loss = torch.clamp(m_pg_loss, -max_loss_value, max_loss_value)
                v_loss = torch.clamp(v_loss, -max_loss_value, max_loss_value)
                termination_loss = torch.clamp(termination_loss, -max_loss_value, max_loss_value)
                meta_termination_loss = torch.clamp(meta_termination_loss, -max_loss_value, max_loss_value)
                # a_entropy_loss = torch.clamp(a_entropy_loss, -max_loss_value, max_loss_value)
                # o_entropy_loss = torch.clamp(o_entropy_loss, -max_loss_value, max_loss_value)
                # m_entropy_loss = torch.clamp(m_entropy_loss, -max_loss_value, max_loss_value)

                loss = a_pg_loss - ent_action_coef_now * a_entropy_loss  \
                    + o_pg_loss - ent_option_coef_now * o_entropy_loss \
                        + m_pg_loss - ent_meta_coef_now * m_entropy_loss \
                        + v_loss * args.vf_coef + termination_loss*0.2 + meta_termination_loss*0.2
        ...
        writer.add_scalar("losses/m_policy_loss", m_pg_loss.item(), global_step_truth)
        ...
        writer.add_scalar("losses/m_entropy", m_entropy_loss.item(), global_step_truth)
        ...
        writer.add_scalar("losses/meta_termination_loss", meta_termination_loss, global_step_truth)
    ...
    if args.warmup:
        ...
        folder_path = f"HOC_policies/{args.env_id}"
