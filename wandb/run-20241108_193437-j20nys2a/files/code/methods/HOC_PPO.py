...
from OC_agents.HOC_PPO_agent import HOCAgent  # Assuming you have saved the OC agent code in option_critic.py
...
def hoc(args):
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        b_m_logprobs = logprobs_meta_options.reshape(-1)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, optionlogprobs, optionentropy = agent.select_option(update_state_rep_o, b_meta_options[mb_inds], b_options[mb_inds])
                ...
                if args.norm_adv:
                    ...
                    mb_o_advantages = (mb_o_advantages - mb_o_advantages.mean()) / (mb_o_advantages.std() + 1e-8)
                    mb_m_advantages = (mb_m_advantages - mb_m_advantages.mean()) / (mb_m_advantages.std() + 1e-8)
                ...
                m_pg_loss1 = -mb_m_advantages * m_ratio
                m_pg_loss2 = -mb_m_advantages * torch.clamp(m_ratio, 1 - args.clip_coef, 1 + args.clip_coef)
                m_pg_loss = torch.max(m_pg_loss1, m_pg_loss2).mean()
        ...
        writer.add_scalar("losses/old_m_approx_kl", old_m_approx_kl.item(), global_step_truth)
        ...
        writer.add_scalar("losses/approx_m_kl", approx_m_kl.item(), global_step_truth)
        ...
        writer.add_scalar("losses/m_clipfrac", np.mean(m_clipfracs), global_step_truth)
